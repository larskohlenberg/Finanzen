...
Eine Transaktion kann ueber `immobilie_id` optional genau **einer** **Immobilie** zugeordnet sein; eine Immobilie kann viele Transaktionen haben. Der Immobilienbezug ist explizit, unabhaengig von der Kategorie und wird nicht aus Gegenpartei, Buchungstext oder einer Immobilienkategorie geraten.
...
Ein **Szenario** ist ein benanntes Buendel expliziter **Annahmen** ueber die Zukunft (M6), das auf dem validierten Datenbestand eine Was-waere-wenn-Rechnung erlaubt (Liquiditaet, Restschuld, Nettovermoegen ueber die Zeit). Es ist eine eigene Entitaet, getrennt vom Ist-Bestand — der Bestand bleibt unberuehrt, das Szenario rechnet nur darauf. In der Nettovermoegens-Projektion werden **Sachwerte** (Immobilien-Marktwert, Edelmetall, Beteiligungen, Rueckkaufswerte) ueber den Horizont **auf dem Stichtags-Zeitwert eingefroren** — keine Wertentwicklung unterstellt (das waere geraten; flach ist die ehrliche Annahme). Dieser eingefrorene Zustand wird in der Ansicht sichtbar gemacht. Eine kuenftige Wertentwicklung waere eine eigene Annahme-Art (YAGNI bis dahin). Welche Positionen ueber die Zeit **existieren**, kann eine Annahme dagegen aendern (Kauf/Erbschaft laesst eine Position entstehen, Verkauf/Schenkung entfallen — siehe *Annahme*).

Status: `entwurf | bestaetigt | verworfen`. `verworfen` ist ein Lebenszyklus-Zustand (bewusst beiseitegelegtes Szenario, zur Doku behalten), **kein** Archiv-/Versionsmechanismus. Das M6-Exit-Kriterium „Annahmen versioniert **oder** mit Gueltigkeit" wird ueber den **Gueltigkeits-Arm** erfuellt: jedes Szenario traegt einen `stand` (Stichtag seiner Annahmen) und eine `reichweite_bis` (Rechenhorizont). Aendert sich die Realitaet, wird das Szenario **in-place** editiert und `stand` hochgezogen (ADR 0002) — es wird **nicht** automatisch archiviert und geklont, es gibt keine Versionsketten. Einen bewussten Vorher-/Nachher-Vergleich legt der Nutzer als zweites, eigenstaendig benanntes Szenario an.

## Annahme

Ein einzelner expliziter Zukunfts-Baustein **innerhalb** eines Szenarios (z. B. eine geplante Sondertilgung, eine Erbschaft, ein Hausverkauf). Eine Annahme ist **immer szenario-gebunden** und wirkt **nie** auf den Ist-Bestand oder die annahmenfreie Basis-Rechnung — alles Zukunftsgerichtete lebt ausschliesslich im Szenario. Jede Annahme traegt eine `qualitaet` (`belegt | geschaetzt | offen`) und moeglichst eine `begruendung` (die menschenlesbare Erklaerung/Notiz des Agenten, in der Annahmen-Tabelle gezeigt — ein Freitextfeld, nicht zwei); die Gesamtqualitaet eines Szenarios ist worst-of ueber seine Annahmen und Eingangsdaten.

M6 kennt **drei Arten** plus einen parametrisierten Gegenbuchungs-Mechanismus statt enumerierter Sonderfaelle: **Einmalzahlung** (einmalig), **neue Regelzahlung** (wiederkehrend) und **Aenderung einer Regelzahlung** (beenden / Betrag aendern). Jede Annahme hat **zwei unabhaengige Beine**: ein **Cash-Bein** (`betrag`, vorzeichenbehaftet, darf 0 sein) und optional ein **Bilanz-Bein** (`gegenbuchung`). Der Nettovermoegens-Effekt ist die **Summe beider Beine**: heben sie sich auf, ist es eine **Umschichtung** (neutral — Kauf, Verkauf, Sondertilgung); ist ein Bein null, ist es ein **echter Zu-/Abgang** (Bar-Erbschaft = nur Cash; Sachwert-Erbschaft/-Schenkung = nur Bilanz-Bein; Konsum = nur Cash negativ).

Die **`gegenbuchung`** referenziert entweder eine **bestehende** Bilanzposition (`ziel_id`, `ziel_typ ∈ darlehen | depot | immobilie | vermoegenswert`) oder deklariert eine **neue** (`neue_position = {bezeichnung, wert}`, fuer Kauf/Erbschaft/Schenkung). Der Gegeneffekt (Restschuld −, Depotwert ∓, Sachwert-Position entsteht/entfaellt), die Ueberzahl-Klemmung und der `depot-vorbehalt` haengen an `ziel_typ` — die Engine hat dahinter typisierte Handler. Diese parametrisierte Form folgt demselben Prinzip wie der *Weitere Vermoegenswert* (erweiterbare Entitaet via `typ` statt Subtyp-Duplikate). Wiederkehrende Gegenbuchungen gibt es nur fuer `darlehen`/`depot` (wiederkehrende Sondertilgung, Sparplan); eine Sachwert-Position entsteht/entfaellt nicht monatlich.

Auch eine als sicher empfundene Sondertilgung ist eine **Annahme**, kein Top-Level-Faktum: die normale Restschuldsicht rechnet weiter nur mit der Sollrate (Basis), eine fest eingeplante Sondertilgung lebt in einem `bestaetigt`-Szenario. Es gibt **keine** zweite, objektgebundene Planwert-Entitaet neben dem Szenario (ein Mechanismus, YAGNI); die Darlehenssicht darf bestehende Sondertilgungs-Annahmen rein informativ ausweisen. Die Grenze zum Faktum: eine **bekannte** Aenderung (z. B. Gehalt ab 60 halbiert) bleibt ein Top-Level-Faktum (zwei aufeinanderfolgende Regelzahlungen, siehe Cashflow-Prognose), die Annahmen sind fuer das **Hypothetische**. Ein rein dokumentarischer Lebensereignis-Marker ohne Geldwirkung gehoert **nicht** dazu (YAGNI); der davon zu unterscheidende **Top-Level-`Ereignis`** (Lebensphasen, personengebunden — siehe *Person*) ist kein M6-Gegenstand, sondern Teil des Lebensphasen-/Vorsorge-Clusters in M7.

## Cash-Realismus-Guardrail

Eine M6-Pruefregel gegen die „zu rosige Zukunft": sie warnt, wenn ein **geschaetzter** Kategorie-Konsumplan (`geschaetzt`-Regelzahlung mit `kategorie_id`) deutlich unter dem **historischen Ist derselben Kategorie** liegt — der geplante Monatsbetrag gegen das durchschnittliche Kategorie-Ist der letzten vollen Monate. **Nur `geschaetzt`-Buckets** werden geprueft; `belegt`-Verpflichtungen (Miete, Versicherung) sind Fakten und werden nicht plausibilisiert. Abzugrenzen vom **Plan-Ist-Abgleich** (M8), der die tatsaechliche Buchung gegen *alle* Planwerte prueft. Der Bar-Anteil bleibt der bewusst akzeptierte blinde Fleck: bar bezahlter, als reine Abhebung abgeflossener Kategorie-Konsum untererfasst das Kategorie-Ist, solange er nicht kategorisiert oder als geschaetzte Bar-Transaktion erfasst wird.

## Vorsorge

Oberbegriff und **eine** Stammdaten-Entitaet (M7) fuer Versicherungen *und* Renten — bewusst **nicht** zwei getrennte Entitaeten, weil ein einzelner realer Vertrag (z. B. private Rentenversicherung, Riester) gleichzeitig beides sein kann und sonst doppelt gepflegt werden muesste. Eine `art` (`lebensversicherung | rentenversicherung | gesetzliche-rente | betriebsrente | riester | ruerup | schutzversicherung | sonstig`) diskriminiert das Verhalten. Jeder Eintrag hat bis zu **zwei unabhaengige Beine** (gleiche Logik wie die M6-**Annahme**):

- **Bilanz-Bein** — ein heutiger **Rueckkaufswert** (als **Zeitwert** `feld = rueckkaufswert`), der als Aktivum ins **Nettovermoegen** zaehlt. Nur **kapitalbildende** Vertraege haben es. `kapitalbildend` (Klassifikations-Bool) heisst: **es gibt einen heute realisierbaren, dem Halter gehoerenden Kapitalwert** (Rueckkaufswert, per Kuendigung zugaenglich) — *nicht* bloss „es gibt einen bekannten Wert". Treiber der Nettovermoegens-Aufnahme ist dieses Merkmal, **nicht** die `art`: `kapitalbildend` ohne Zeitwert ⇒ sichtbare Luecke (`null`/`offen`); `!kapitalbildend` ⇒ nie eine Position. Loest die bAV-Kante (Direktversicherung mit RKW vs. Direktzusage ohne). Bei **Auszahlung** (Faelligkeit oder Tod) im Szenario wird der eingefrorene Rueckkaufswert ueber eine **Gegenbuchung `ziel_typ: vorsorge`** abgebaut (gleicher Mechanismus wie Immobilien-/Depot-Abbau) — sonst Doppelzaehlung (Aktivum *und* ausgezahltes Cash).
- **Income-Bein** — eine **erwartete Leistung**, die **nie** ins Nettovermoegen zaehlt (eine Anwartschaft ist kein heutiges Vermoegen) und nur in **Szenarien** als kuenftige Regelzahlung oder Einmalzahlung wirkt. Bei **Kapitalwahlrecht** kann ein Vertrag beide Auszahlungsformen anbieten; die Wahl ist ein `kapitalwahl`-Status (`offen | monatsrente | kapital`) und wird — solange relevant — erst im Szenario realisiert (Kapital → `einmalzahlung`-Annahme, Verrentung → unbefristete `regelzahlung-neu`-Annahme).

Drei getrennte **Zeitwert-`feld`er** halten die drei nicht-vermischbaren Werte: `rueckkaufswert` (heute, Aktivum), `erwartete_kapitalleistung` (einmalig bei Faelligkeit) und `erwartete_rente` (monatlich, lebenslang ab Faelligkeit). **Erwartete Leistungen werden als Netto-Cash geschaetzt** (was real aufs Konto kommt — konsistent mit dem Netto-Gehalt als Regelzahlung); die Bruttozahl der Renteninformation und der KV/PV/Steuer-Abschlag stehen in der Bemerkung. Kein Steuer-Engine (YAGNI). Solche Netto-Schaetzungen veralten (z. B. Gesetzesaenderung) und brauchen periodische **Wiedervorlage** (siehe `geprueft`/`standdatum`). Eine reine **Schutzversicherung** (KFZ, Haftpflicht) hat **kein** Bein — nur einen verknuepften **Beitrag** (normale Regelzahlung). Die gesetzliche Rente hat **nur** das Income-Bein (`erwartete_rente`).

**Zwei Verweis-Pfade in Szenarien** (Status `laufend`/`geplant` ist die Grenze):
- **Beitrag und *laufende* Leistung sind Regelzahlungen** und stecken damit schon in der Szenario-**Basis**. Verknuepfung **einseitig**: die **Regelzahlung traegt `vorsorge_id`** (wie `darlehen_id`), die Vorsorge fuehrt **keine** Rueckliste — die Rueckrichtung ist per Filter ableitbar. Ein Vertrag hat ueber die Jahre mehrere Beitrags-Regelzahlungen (Zwei-RZ-Muster pro Jahreswechsel); die aktuelle Praemie ist die RZ mit offenem/spaetestem `aktiv_bis`.
- **Geplante Anwartschaften** (noch nicht laufend; Renteneintritt ist eine Lebensentscheidung) fliessen **nur** ueber ein Szenario ein — als Annahme-Art `vorsorge-leistung`, die `vorsorge_id` + Arm referenziert; die Engine loest den `erwartete_*`-Zeitwert zur Rechenzeit auf und deckelt die Qualitaet nach `geprueft`.

**Auszahlungs-Anlass (Erlebensfall/Todesfall) lebt im Szenario, nicht in der Entitaet.** Ein Szenario „Todesfall am …" realisiert die todesbedingten Leistungen, ein „Ruhestand"-Szenario die erlebensbedingten — der Agent schreibt die passenden Annahmen, es gibt **keinen** Event-/Beneficiary-Engine. **Hinterbliebenenrenten** (Witwen-/Waisenrente) sind gesetzliche Derivat-Ansprueche der Hinterbliebenen und werden als **Szenario-`regelzahlung-neu`** modelliert (Waisenrente mit `aktiv_bis` = Altersgrenze), **nicht** als Vertrags-Stammdatum. Eine **Risikolebensversicherung** ist `!kapitalbildend` (kein Rueckkaufswert), traegt aber ein **Income-Bein** (Todesfallsumme als `erwartete_kapitalleistung`) — Beleg dafuer, dass die zwei Beine unabhaengig sind.

**Nachfolge / Wechsel:** ein Versicherungswechsel wird ueber `ersetzt_vorsorge_id` auf der **neuen** Vorsorge ausgedrueckt (einseitig, Rueckrichtung per Filter). Der **Wechsel-Check** (ein schaerferer Verwandter der **Cash-Realismus-Guardrail**) feuert nur fuer Beitraege, deren `aktiv_bis` in der **Zukunft innerhalb des Horizonts** liegt: kein/spaeterer Nachfolger ⇒ Warnung „Wegfall/Deckungsluecke". Vergangene Kuendigungen warnen nie (ihr Wegfall ist schon Realitaet). Versicherung-Lebenszyklus: `aktiv | gekuendigt | ruhend`; Renten-Anwartschaft: `geplant | laufend | beendet`.
...
`qualitaet`: `belegt | geschaetzt`. Datenqualitaet sitzt **bei dem Wert, den sie qualifiziert** — beim Zeitwert die einzelne Zeile, bei der Regelzahlung deren eigener `betrag`. Eine Entitaet, die **keinen** eigenen Wert traegt (z. B. **Vorsorge** — alle Werte liegen in Zeitwerten), traegt darum auch **keine** `qualitaet`. Sie wird nie zu einer Entitaet aggregiert.
...
- **Stammdaten** (Immobilie, Darlehen, Versicherung, Rente, Sozialleistung, **Regelzahlung**): optionales `quelle_hinweis` (Pfad oder Freitext) plus optionales `quelle_standdatum`. Bei einer `belegt`-Regelzahlung verweist `quelle_hinweis` auf den Vertrag bzw. die Jahres-Beitragsinfo.
...
Eine Regelzahlung traegt eine `qualitaet` (`belegt | geschaetzt`), **orthogonal zum `status`** (Status = Herkunft/Bestaetigung, Qualitaet = Sicherheit des Betrags). `belegt` = eine **vertragliche Verpflichtung** mit exaktem Betrag (Miete, Mobilfunk, Leasing), meist mit `aktiv_bis` und einem `quelle_hinweis` (Pfad zum Vertrag / zur Jahres-Beitragsinfo). `geschaetzt` = ein **Kategorie-Konsumplan** ohne Vertrag, der die Zukunft auf **Kategorie-Granularitaet** plant (z. B. „Lebensmittel monatlich −500", `kategorie_id` gesetzt, keine `gegenpartei`) — welche Gegenpartei real bucht (MusterladenB/Lidl/MusterladenC) ist zukuenftig offen und irrelevant. Eine bekannte jaehrliche Aenderung (z. B. Versicherungsbeitrag) bleibt eine **Folge zweier Regelzahlungen** (je eigenes `aktiv_bis` + `quelle_hinweis`), kein eigener Mechanismus. Diese Felder machen Planwerte **abgleich-faehig** fuer den Plan-Ist-Abgleich (M8); M6 legt nur die Naht, nicht die Maschine.

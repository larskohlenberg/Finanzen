...
// Menschliche Akte sind fuer die Wiedervorlage-Pruefung tabu — recategorize.mjs
// fasst sie ohnehin nie an, also kann ein Kandidat dort auch nichts ausloesen.
...
export function probelauf({ transaktionen, bestandsRegeln, kandidaten, kategorieIds = null, aenderung = false }) {
  ...
  const pro_regel = Object.fromEntries(kandidaten.map((r) => [r.regel_id, { kategorie_id: r.kategorie_id, treffer: 0, summe_cents: 0, beispiele: [] }]));
  ...
  for (const tx of transaktionen) {
    ...
    // Nur Buchungen betrachten, an denen ein Kandidat ueberhaupt beteiligt ist.
    if (!nachher.matched_regeln.some((id) => kandidatIds.has(id))) continue;

    if (nachher.conflict) {
      // Ein Konflikt, den es vorher schon gab, ist nicht dem Kandidaten anzulasten.
      const vorher = categorize(tx, bestandsRegeln);
      if (!vorher.conflict) {
        neue_konflikte.push({
    ...
    if (tx.kategorisierung_status === "bestaetigt" && !istUnantastbar(tx) && nachher.kategorie_id !== tx.kategorie_id) {
      wiedervorlagen.push({
        transaktion_id: tx.transaktion_id,
        buchungsdatum: tx.buchungsdatum,
        betrag: tx.betrag,
    ...
    if (tx.kategorisierung_status !== "offen") continue;
    ...
    for (const id of nachher.matched_regeln) {
      ...
      if (!eintrag) continue;
      eintrag.treffer += 1;
      eintrag.summe_cents += Math.round(parseFloat(tx.betrag) * 100);
...
function bericht(out) {
  ...
  zeilen.push(`Offene Buchungen, die die Kandidaten treffen: ${out.treffer}`);
  zeilen.push("");
  for (const [id, r] of Object.entries(out.pro_regel)) {
    zeilen.push(`${id}  ${String(r.treffer).padStart(4)}x  ${euro(r.summe_cents).padStart(12)} EUR  -> ${r.kategorie_id}`);
...
Exit-Code 2, wenn neue Regelkonflikte, Wiedervorlagen oder Strukturfehler auftreten.

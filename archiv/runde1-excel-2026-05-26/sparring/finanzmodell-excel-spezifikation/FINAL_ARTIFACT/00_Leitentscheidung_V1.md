...
- sichtbare Dashboard-Zielstellen fuer Liquiditaet, Cashflow und Reichweite, aber noch ohne Finanzwert oder Entscheidungsanspruch,
...
Dieses Gate beweist nur, dass die Struktur die spaetere Familienfrage nicht blockiert: IDs, Quellen, Import, Rohumsatz, Modellumsatz, Checks, Kontrollstatus und Dashboardstatus sind referenzierbar verbunden, und Unsicherheit bleibt sichtbar. Der Kontrollstatus ist dabei bereits ein erstes enges Produktverhalten: Die Mappe zeigt, warum sie noch nicht belastbar ist. Das Gate beweist aber noch nicht, dass Cashflow, Liquiditaet oder Reichweite fachlich richtig berechnet werden, und es darf dem Nutzer keinen finanziellen Nutzwert zuschreiben.

Eine rote oder gelbe Mappe ist in diesem Gate ausdruecklich richtig, wenn der Status nicht nur gesetzt, sondern auf mindestens einen vorhandenen Check und dessen betroffene Quelle, Annahme, Importlauf- oder Kontrollspurzeile rueckverweist. Gruen ist in Task 1 nicht akzeptiert, weil noch keine bestandene Verifikation und keine Formellogik fuer Finanzkennzahlen vorliegen.
...
- minimale Dashboard-Statusfelder, die einen roten oder gelben Startzustand mit Check-, Quellen-, Annahmen-, Import- oder Kontrollspurbezug sichtbar machen,
...
Die Grenze zu Task 3 wird nicht ueber Selbstauskunfts-Felder im Produktcode gezogen, sondern ueber getrennte Testebenen und Modulgrenzen: Task 1 prueft Struktur, Referenzen, Kommentare, Validierungen, Seed-Abdeckung sowie den roten oder gelben Kontrollstatus. Er importiert keine Formellogik, erzeugt keine berechneten Kennzahlen aus Umsatz-, Annahmen- und Szenariodaten und zeigt keine Finanzwerte fuer Liquiditaet, Cashflow oder Reichweite als Entscheidungshilfe. Die erste Formellogik beginnt erst dort, wo aus Umsatz-, Annahmen- und Szenariodaten berechnete Werte entstehen: `Liquiditaet_heute`, `Cashflow_Monat_ist`, `Cashflow_Monat_erwartet`, `Cashflow_Monat_gesamt`, Runway-Projektion und die dazugehoerigen Check-Ausloeser. Diese Logik gehoert in Task 3 nach `formulas.mjs` und `formulas.test.mjs`. Task 1 darf ihre spaeteren Zielspalten strukturell vorbereiten, aber nicht ihre fachliche Berechnung, ihren Wert oder ihren Nutzwert vorwegnehmen.

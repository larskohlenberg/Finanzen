...
| `thin_slice_gate` | Minimaldaten fuer Personen, Haushalt, Kategorien, Konto, Szenario, Annahmen und Quelle; ein Importlauf; wenige Roh- und Modellumsaetze; Dashboard-Zielstellen ohne Finanzwert; ein Check mit betroffener Quelle, Annahme, Import- oder Kontrollspur-Referenz; Dashboardstatus Rot oder Gelb; Kontrollstatus `nicht_ausgefuehrt` oder `nicht_pruefbar` | Struktur-, Referenz-, Kontrollstatus- und Sichtbarkeitsnachweis, bevor die Breite aller Muss-Tabellen voll ausgebaut und bevor Formellogik geschrieben wird. Kein Finanznutzwertnachweis. |
...
Task 1 beweist den V1-Kern nicht durch vollstaendige Formeln, nicht durch eine fertige Entscheidungssicht und nicht durch behaupteten Finanznutzwert. Er beweist, dass die spaetere Entscheidungssicht strukturell tragfaehig, referenzierbar und nicht unsichtbar-gruen ist. Der rote oder gelbe Kontrollstatus gilt als erstes enges Produktverhalten, solange er auf Checks, Quellen und negative Sichtbarkeit begrenzt bleibt. Der Durchstich ist akzeptiert, wenn `workbookSpec` und `seedData` gemeinsam diese Mindestkette beschreiben:

1. `01_Personen`: mindestens `P01` als Person und `HH` als Haushalt, mit offenem oder belegtem Status sichtbar.
...
12. `00_Dashboard`: ein Startstatus Rot oder Gelb mit Kontrollstatus, naechster Aktion, sichtbaren Zielstellen fuer spaetere Kennzahlen und Rueckverweis auf mindestens einen vorhandenen Check.

Task 1 testet keine Liquiditaets-, Cashflow- oder Reichweitenzahl, auch nicht als harmlos wirkenden Seed-Wert. Er darf die spaeteren Dashboardfelder, Rollen, Kommentare und Statusanzeigen strukturell vorbereiten. Die Berechnung aus Umsaetzen, Annahmen und Szenarien ist noch nicht Ziel von Task 1.
...
- `startDashboard.Startzustand_Typ = seeded_control_status`; Finanzkennzahlen sind leer, `nicht_berechnet` oder als Zielstelle ohne Wert gekennzeichnet.

Die Grenze ist verbindlich:

- Task 1 trennt drei Testebenen: Strukturvertrag, Kontrollstatus-/Sichtbarkeitsverhalten und spaetere Finanzkennzahlen-/Formellogik.
- Task 1 testet Struktur, Reihenfolgen, Rollen, Validierungen, Kommentare, Pflichtspalten und Seed-Abdeckung.
- Task 1 testet den roten oder gelben Kontrollstatus, die konkrete Referenzkette von Konto, Quelle, Importlauf, Rohumsatz, Modellumsatz, Check und Dashboardstatus sowie die bewusste Abwesenheit berechneter Finanzkennzahlen.
...
- Task 1 darf keinen Nutzerwert fuer Liquiditaet, Cashflow oder Reichweite behaupten; er zeigt nur, dass die spaeteren Felder, IDs, Checks und Statusreferenzen sichtbar verbunden werden koennen und dass fehlende Belastbarkeit nicht unsichtbar wird.
...
Task-1-Dashboardfelder sind als Struktur- und Kontrollstatus-Startzustand zu kennzeichnen. Sie duerfen Rot oder Gelb anzeigen, weil offene Kategorie, fehlende Verifikation oder Platzhalter-Annahmen sichtbar sind. Der Status muss auf vorhandene Checks und deren betroffene Quellen, Annahmen, Import- oder Kontrollspurzeilen rueckverweisen; ein bloss gesetzter Warnstatus reicht nicht. Liquiditaets-, Cashflow- und Reichweitenwerte bleiben in Task 1 leer, `nicht_berechnet` oder als Zielstelle ohne Wert markiert. Die spaetere Berechnung der Kennzahlen erfolgt in Task 3.
...
Ein statusbegruendender Check fuellt mindestens die Referenz, die seinen Befund primaer begruendet. Wenn ein Startbefund fachlich nur als Zusammenhang aus Quelle, Importlauf, Annahme oder Kontrollspur pruefbar ist, darf er mehrere dieser vier konkreten Felder fuellen. Es gibt keine generische Referenzmatrix, keine Liste aller moeglichen Zieltabellen und keine Pflicht, jede Check-Zeile kuenstlich mit mehreren Bezuegen aufzublaehen. Fuer Task 1 genuegt mindestens ein vorhandener Check, dessen konkrete Referenzfelder auf vorhandene Quelle, Annahme, Importlauf- oder Kontrollspur-Zeilen zeigen und dadurch den roten oder gelben Dashboard-Startstatus erklaeren.
...
Task 1 baut `sheetOrder`, `tableBuildOrder`, alle Muss-Tabellen strukturell und die Seeds, die fuer den ersten roten oder gelben Dashboard- und Kontrollstatus noetig sind. Dieser Startzustand ist ein Struktur-, Referenz-, Kontrollstatus- und Sichtbarkeitsnachweis, kein fertiger Finanznutzwertnachweis. Task 1 darf Dashboardfelder als Zielstellen und Statuszustand definieren; Formeltiefe, Parserdetails und `.xlsx`-Export folgen danach.

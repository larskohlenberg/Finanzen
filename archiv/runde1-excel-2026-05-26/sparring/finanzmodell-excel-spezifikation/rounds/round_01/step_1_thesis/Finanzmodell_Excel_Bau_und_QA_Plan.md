...
## These-Schaerfung: Umsetzungsgates fuer Task 1

Diese Ergaenzung macht den Bauplan fuer den naechsten Codex-Lauf entscheidbarer. Der erste Entwicklungsschritt darf nicht direkt mit Formeln beginnen; zuerst muss der maschinelle Vertrag zwischen Spezifikation, Builder und Tests stehen.

### Task 0: Laufzeit- und Tooling-Check

Vor Task 1 wird ein kurzer, nicht fachlicher Check ausgefuehrt:

1. Node-Version verfuegbar.
2. `@oai/artifact-tool` verfuegbar oder begruendet nicht verfuegbar.
3. Falls `@oai/artifact-tool` nicht verfuegbar ist: `exceljs` wird primaerer Inspector fuer lokale Artefakt-Checks; die Codex-spezifische Inspector-Klasse bleibt als Interface-Implementierung vorgesehen.
4. Schreibpfad `workbook-build/` ist frei oder bewusst fortsetzbar.
5. Es wird kein `.xlsx` exportiert, bevor mindestens `minimal` gruen verifiziert ist.

Akzeptanz: Der Check endet mit einer kurzen technischen Notiz in `workbook-build/README.md`, nicht mit einer fachlichen Designentscheidung.

### Build-Gate A: Spezifikation kompilierbar

Dieses Gate ist erreicht, wenn:

- `workbookSpec.mjs` alle dokumentierten Blattnamen in Reihenfolge enthaelt.
- Alle `muss`-Tabellen aus dem Datenmodell in `workbookSpec.tables` existieren.
- Jede `muss`-Tabelle Spalten, Primaerschluessel, Spaltenrollen, Validierungen und Spaltenkommentare hat.
- Alle festen Status- und Typwerte genau in der dokumentierten Schreibweise enthalten sind.
- Die Seed-Daten fuer `01_Personen`, `02_Kategorien`, `40_Szenarien`, `42_Annahmen`, `72_Agent_Pruefregeln` und `99_Checks` maschinell vorliegen.

Nicht akzeptiert: Ein Workbook-Skelett, das Blaetter anlegt, aber Tabellenrollen, Validierungen oder Seed-Daten spaeter nachreichen will.

### Build-Gate B: Import reproduzierbar

Dieses Gate ist erreicht, wenn:

- Der CSV-Parser die dokumentierten deutschen Datums- und Betragsformate parst.
- Der Parser Import-Metadaten, Rohumsatzzeilen und Importlauf-Protokoll getrennt erzeugt.
- `Zeilenhash`, `Import_ID`, `Rohumsatz_ID` und `Transaktion_ID` deterministisch entstehen.
- Ein zweiter Import derselben Datei keine doppelten Netto-Rohdaten erzeugt.
- Der Parser keine Kategorie, Regelzahlung, Transferregel oder `Person_ID` final bestaetigt.

Nicht akzeptiert: Parserlogik, die aus Bequemlichkeit `HH`, `geprueft`, `bestaetigt` oder finale Kategorien setzt.

### Build-Gate C: Workbook nutzbar, aber ehrlich

Dieses Gate ist erreicht, wenn die minimal gebaute Mappe:

- alle `muss`-Blaetter und `muss`-Tabellen enthaelt,
- das Dashboard mit `Gelb` startet, solange Platzhalter und offene Basisdaten existieren,
- keine Formel-Fehler in Key-Ranges enthaelt,
- offene Punkte sichtbar in `99_Checks`, `60_Warnungen`, `71_Agent_Auftraege` oder `73_Agent_Vorschlaege` zeigt,
- auf Detailblaettern oben einen Kennzahlen-/Statusbereich hat,
- Eingaben, Formeln, Status, Quellen und technische Felder visuell unterscheidbar macht.

Nicht akzeptiert: Eine gruene Startmappe mit offenen Geburtsdaten, Platzhalterannahmen oder ungeprueften Quellen.

### Build-Gate D: Agenten-Compliance pruefbar

Dieses Gate ist erreicht, wenn simulierte Agentenlaeufe zeigen:

- Import-Agent schreibt keine finalen Regelzahlungen oder Transferregeln.
- Pruef-Agent erzeugt Vorschlaege/Warnungen, aber keine fachlichen Endentscheidungen.
- Recherche-Agent ueberschreibt keine belegten/geprueften Annahmen still.
- Umsetzungs-Agent setzt nur angenommene und eindeutige Vorschlaege um.
- Wiederholte Laeufe bleiben idempotent.

Nicht akzeptiert: Agenten-Tests, die nur pruefen, dass Tabellen geschrieben werden, aber verbotene Statusaenderungen nicht erkennen.

### Konkrete Reihenfolge fuer den naechsten Entwicklungsstart

1. `workbook-build/README.md` mit Ziel, Tooling-Check und Run-Befehlen anlegen.
2. `package.json` mit `test`, `build`, `verify` vorbereiten.
3. `workbookSpec.test.mjs` schreiben und rot sehen.
4. `workbookSpec.mjs` minimal, aber vollstaendig fuer `muss`-Tabellen implementieren.
5. `seedData.mjs` auslagern, sobald Seed-Zeilen mehr als triviale Konstanten sind.
6. `csvStartimportParser.test.mjs` schreiben und rot sehen.
7. Parser implementieren.
8. `artifactVerifier.mjs` als Stub nur dort anlegen, wo Imports sonst blockieren.
9. Erst danach Workbook-Builder und Formeln aufbauen.

Regel: Jeder Task endet mit einem pruefbaren roten oder gruenen Zustand. Kein Task endet nur mit "Dokumentation erweitert".

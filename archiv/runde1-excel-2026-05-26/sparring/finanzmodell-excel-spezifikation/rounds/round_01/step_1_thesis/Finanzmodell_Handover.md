...
## These-Schaerfung: Naechster Start als kleine Sequenz

Die naechste Session sollte nicht mit "baue die Excel-Datei" starten, sondern mit einer engen Build-Sequenz:

1. Tooling-Check: Node, Paketverfuegbarkeit, `@oai/artifact-tool` oder `exceljs`-Fallback.
2. `workbook-build/` anlegen.
3. `README.md` mit Run-Befehlen und Tooling-Ergebnis schreiben.
4. `package.json` mit `test`, `build`, `verify`.
5. `workbookSpec.test.mjs` schreiben und RED sehen.
6. `workbookSpec.mjs` fuer alle `muss`-Tabellen aus dem Datenmodell implementieren.
7. Seed-Daten und Validierungen auslagern.
8. Erst danach CSV-Parser-Tests schreiben.

Blocker fuer den Start:

- Wenn keine lokale Workbook-Render-/Inspect-Moeglichkeit verfuegbar ist, darf trotzdem `workbookSpec` und CSV-Parser gebaut werden; der Export der `.xlsx` wartet dann auf den Verifier-Pfad.
- Wenn die Girokonto-Fixture noch fehlt, darf Task 1 trotzdem starten; Task 2 bekommt einen klaren Skip/RED mit Hinweis auf `tests/fixtures/csv/girokonto_test.csv`.

Erfolg der naechsten Session:

- Nicht "erste Excel-Datei existiert", sondern "`workbookSpec` ist testbar, `muss`-Tabellen sind maschinell definiert, und der CSV-Parser kann als naechster isolierter Schritt beginnen".

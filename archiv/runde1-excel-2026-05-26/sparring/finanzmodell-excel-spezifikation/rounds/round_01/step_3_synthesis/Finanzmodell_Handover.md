...
- Version 1 ist auf einen kleinen Nutzwertkern begrenzt.
...
- `workbookSpec.mjs` bleibt ein Strukturvertrag fuer den V1-Kern und dupliziert nicht die gesamte Markdown-Spezifikation.
- Excel bleibt Nutzeroberflaeche und Auditspur.
- Build-, Fixture- und Inspector-Details bleiben im Ordner `workbook-build/`.
- Agentenworkflow bleibt wichtig, soll aber den ersten Cashflow-/Liquiditaetsnutzen nicht dominieren.

## Naechster Umsetzungsschritt

Die naechste Session startet mit dem technischen Bau des V1-Kerns:
...
6. `workbookSpec.mjs` fuer die `muss`-Tabellen des V1-Kerns implementieren.
...
- Startdaten fuer Personen, Kategorien, Szenarien, Annahmen, Pruefregeln und Checks existieren.
...
- `workbookSpec.mjs` den V1-Kern strukturell beschreibt.

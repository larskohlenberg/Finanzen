...
Die Excel-Datei ist ein Build-Artefakt, die Nutzeroberflaeche und die fachliche Auditspur. Build-, Fixture-, Inspector- und Subagenten-Details bleiben in `workbook-build/`. Der Master speichert daraus nur knappe Nachweisanker. Die fachliche Logik bleibt in den Markdown-Dateien dokumentiert. Code darf fachliche Luecken nicht still schliessen.
...
1. `workbookSpec.mjs`: Blatt-, Tabellen-, Spalten-, Validierungs-, Kommentar- und Seed-Struktur fuer den V1-Kern inklusive `98_Kontrollspur`.
...
7. `artifactVerifier.mjs`: unabhaengige Pruefschicht mit Build-Zusammenfassung.
8. `agentComplianceHarness.mjs`: Tests fuer Rollen- und Statusgrenzen, mit verdichteter Laufzusammenfassung fuer den Master.
...
- Spaltenrollen.
- Datenvalidierungen.
- Seed-Zeilen fuer Startkataloge und Checks.
...
- Rohlogs, Inspector-Snapshots oder Subagenten-Antworten.
...
- Seed-Daten fuer `01_Personen`, `02_Kategorien`, `40_Szenarien`, `42_Annahmen`, `98_Kontrollspur`, `99_Checks` und zentrale Vorschlagsstatus maschinell vorliegen.
...
- `Import_ID`, `Rohumsatz_ID`, `Transaktion_ID`, `Lauf_ID` und `Zeilenhash` deterministisch sind.
...
- `98_Kontrollspur` die letzte Build-Verifikation und Laufanker sichtbar macht.
- Eingaben, Formeln, Status, Quellen, technische Felder und Kontrollspur visuell unterscheidbar sind.

### Gate E: Agentengrenzen pruefbar und im Master verankert
...
- Compliance-Ergebnisse nicht als eigene Nutzerplattform erscheinen, aber `98_Agentenlaeufe`, `73_Agent_Vorschlaege`, `60_Warnungen` und `99_Checks` die relevanten Befunde verankern.
...
test("control trace tables are present but minimal", () => {
  assert.ok(workbookSpec.tables["98_Build_Verifikation"]);
  assert.ok(workbookSpec.tables["98_Agentenlaeufe"]);
  assert.ok(workbookSpec.tables["98_Artefakt_Referenzen"]);
...
`workbookSpec.mjs` definiert die V1-Kerntabellen und die strukturellen Metadaten. `seedData.mjs` enthaelt Startkataloge, Kontrollstatus-Werte und Check-Definitionen.
...
test("parser returns importlauf, raw rows and initial model rows", async () => {
  const result = await parseGiroCsv("./tests/fixtures/csv/girokonto_test.csv", {
    ...
    laufId: "RUN-20260518-001",
    quellkontoId: "KTO001",
  });
  assert.ok(result.importlauf);
  assert.equal(result.importlauf.Lauf_ID, "RUN-20260518-001");
...
```text
Kontrollstatus =
  Rot, wenn letzte Verifikation fehlgeschlagen oder Agentenlauf ohne Compliance-Status
  Gelb, wenn Verifikation fehlt, nicht pruefbar ist oder offene Kontrollwarnungen bestehen
  Gruen, wenn letzte Verifikation bestanden und keine Kontrollspur-Fehler offen sind
```
...
| `build_verification_missing` | Kontrollspur ohne bestandene Verifikation |
...
- Kontrollstatus korrekt.
- Checks reagieren auf Fixtures.
- Warnungs- und Vorschlagsfingerprints bleiben stabil.
- `CHK-PERS-01`, `CHK-PERS-02`, `CHK-PERS-03`, `CHK-SUG-01`, `CHK-BLD-01` und `CHK-RUN-01` sind als Seed-Checks vorhanden und auswertbar.
...
### Task 7: Agenten-Compliance ausserhalb der Nutzeroberflaeche, im Master verankert
...
- Pruef-/Analyse-Agent darf Vorschlaege, Warnungen und Laufanker schreiben.
...
- `98_Kontrollspur` mit knappem Statusbereich und drei Tabellen, nicht als Log-Dump.

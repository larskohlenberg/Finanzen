...
## Runde-3-Entscheidungen fuer die `workbookSpec`-Startreihenfolge

| Nr. | Frage | Entscheidung | Begruendung |
|---:|---|---|---|
| 44 | Gibt es eine getrennte Spec-Startreihenfolge? | Ja. `sheetOrder` bleibt nutzerorientiert, `tableBuildOrder` wird dependency-orientiert. | Das Dashboard soll sichtbar vorne stehen, aber technisch zuletzt aus stabilen Quellen gebaut werden. |
...
4. `workbookSpec.mjs` mit `sheetOrder` und `tableBuildOrder` gemaess `Finanzmodell_WorkbookSpec_Startreihenfolge.md` implementieren.
5. Seed-Daten in `seedData.mjs` auslagern.
6. Danach erst CSV-Parser-Tests beginnen.

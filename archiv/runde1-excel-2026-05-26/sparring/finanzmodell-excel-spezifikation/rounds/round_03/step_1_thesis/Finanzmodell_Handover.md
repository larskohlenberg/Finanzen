...
1. `00_Leitentscheidung_V1.md`  
   Beschreibt die Produktgrenze von Version 1: kleiner entscheidungsrelevanter Kern mit minimaler Kontrollspur statt vollstaendiger Architektur.

2. `Finanzmodell_WorkbookSpec_Startreihenfolge.md`  
   Legt `sheetOrder`, `tableBuildOrder` und das erste `workbookSpec`-Startpaket fest.

3. `Finanzmodell_Entscheidungsprotokoll.md`  
   Enthaelt die aktiven Entscheidungen und Begruendungen.

4. `Finanzmodell_Datenmodell.md`  
   Enthaelt Blattstruktur, Tabellen, Felder, Statuswerte, Schluessel, Checks, Kontrollspur und Modellwirkungen.

5. `Finanzmodell_Agentenworkflow.md`  
   Enthaelt Agentenrollen, Methodiken, Output-Vertraege, Laufanker und Compliance-Grenzen.

6. `Finanzmodell_Excel_Bau_und_QA_Plan.md`  
   Enthaelt TDD-Reihenfolge und Build-Gates fuer die erste Excel-Mappe.
...
6. `workbookSpec.mjs` mit `sheetOrder` und `tableBuildOrder` gemaess `Finanzmodell_WorkbookSpec_Startreihenfolge.md` implementieren.
7. `98_Kontrollspur` nur als ein Blatt mit drei Tabellen definieren: `98_Build_Verifikation`, `98_Agentenlaeufe`, `98_Artefakt_Referenzen`.
...
- Startdaten fuer Personen, Kategorien, Szenarien, Annahmen, Kontrollstatus und Checks existieren.
...
- `sheetOrder` und `tableBuildOrder` gemaess Startreihenfolge getestet sind.

...
1. **Thin-Slice-Gate:** Ein minimaler Durchstich zeigt nach wenigen Testdaten einen roten oder gelben Dashboardstatus. Dafuer reichen minimale Stammdaten, ein Importlauf, wenige Roh- und Modellumsaetze, eine Cashflow-/Liquiditaetskennzahl, ein Check und ein Kontrollstatus.
...
test("thin slice has enough seeds to produce a red or yellow dashboard", () => {
  assert.ok(seedData.personen.length >= 1);
  assert.ok(seedData.kategorien.length >= 1);
  assert.ok(seedData.konten.length >= 1);
  assert.ok(seedData.szenarien.some((row) => row.Status === "aktiv"));
  assert.ok(seedData.annahmen.length >= 1);
...
- `workbookSpec.mjs` definiert `sheetOrder`, `tableBuildOrder`, `tables` und `statusSets`.
- `seedData.mjs` enthaelt Personen, Kategorien, Konten, Szenarien, Annahmen, Kontrollstatus-Werte, Vorschlagsstatus-Werte, Agentenrollen, Check-Definitionen und `startDashboard`.

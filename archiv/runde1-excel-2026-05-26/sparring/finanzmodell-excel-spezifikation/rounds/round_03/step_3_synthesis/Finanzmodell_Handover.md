...
1. `00_Leitentscheidung_V1.md`  
   Beschreibt die Produktgrenze von Version 1: kleiner entscheidungsrelevanter Kern mit erstem Nutzwert-Gate und minimaler Kontrollspur statt vollstaendiger Architektur.

2. `Finanzmodell_Entscheidungsprotokoll.md`  
   Enthaelt die aktiven Entscheidungen und Begruendungen, einschliesslich Task-1-Schnitt und Wegfall der separaten Startreihenfolge-Datei.

3. `Finanzmodell_Datenmodell.md`  
   Enthaelt Blattstruktur, Tabellen, Felder, Statuswerte, Schluessel, Checks, Kontrollspur, Modellwirkungen sowie `sheetOrder` und `tableBuildOrder`.
...
5. `Finanzmodell_Excel_Bau_und_QA_Plan.md`  
   Enthaelt TDD-Reihenfolge, Thin-Slice-Gate und Build-Gates fuer die erste Excel-Mappe.
...
- Task 1 startet mit einem vertikalen Nutzwert-Gate und endet mit einem strukturellen Vertrag fuer alle Muss-Tabellen.
...
6. Thin-Slice-Test formulieren: minimale Seeds erzeugen Dashboardstatus Rot oder Gelb, nicht Gruen.
7. `workbookSpec.mjs` mit `sheetOrder` und `tableBuildOrder` gemaess `Finanzmodell_Datenmodell.md` implementieren.
8. alle Muss-Tabellen strukturell mit Spaltenrollen, Validierungen, Kommentaren, Update-Modi und Startstatus definieren.
9. `98_Kontrollspur` nur als ein Blatt mit drei Tabellen definieren: `98_Build_Verifikation`, `98_Agentenlaeufe`, `98_Artefakt_Referenzen`.
10. Seed-Daten in `seedData.mjs` auslagern.
11. Danach erst CSV-Parser-Tests beginnen.
...
- Startdaten fuer Personen, Kategorien, Konten, Szenarien, Annahmen, Kontrollstatus und Checks existieren.

...
| 50 | Was beweist das erste Nutzwert-Gate? | Minimaldaten, Importlauf, Roh-/Modellumsaetze, eine Cashflow-/Liquiditaetskennzahl, ein Check, Dashboardstatus Rot/Gelb und Kontrollstatus. | Der erste rote oder gelbe Dashboardbefund zeigt, dass die Mappe echte Unsicherheit sichtbar macht. |
...
4. Thin-Slice-Test fuer Rot/Gelb-Dashboardstatus aus minimalen Seeds formulieren.
5. `workbookSpec.mjs` mit `sheetOrder` und `tableBuildOrder` gemaess Datenmodell implementieren.
6. alle Muss-Tabellen strukturell mit Spaltenrollen, Kommentaren, Validierungen und Startstatus beschreiben.

...
| `thin_slice_gate` | Minimaldaten fuer Personen, Kategorien, Konto, Szenario, Annahmen und Quelle; ein Importlauf; wenige Roh- und Modellumsaetze; ein expliziter Startwert fuer Liquiditaet oder Cashflow; ein Check; Dashboardstatus Rot oder Gelb; Kontrollstatus `nicht_ausgefuehrt` oder `nicht_pruefbar` | Erster Nutzwertnachweis, bevor die Breite aller Muss-Tabellen voll ausgebaut wird. |
...
Task 1 beweist den V1-Kern nicht durch vollstaendige Formeln, sondern durch einen konkret benannten Struktur- und Seed-Durchstich. Der Durchstich ist akzeptiert, wenn `workbookSpec` und `seedData` gemeinsam diese Mindestkette beschreiben:
...
11. `99_Checks`: mindestens ein Build- oder Datenqualitaetscheck, der Rot oder Gelb begruendet.
12. `00_Dashboard`: ein Startstatus Rot oder Gelb mit Kontrollstatus und naechster Aktion.

Die in Task 1 getestete Kennzahl darf ein expliziter Seed- oder Start-Dashboardwert sein, wenn er als Startzustand gekennzeichnet ist. Die Berechnung aus Umsaetzen, Annahmen und Szenarien ist noch nicht Ziel von Task 1.
...
Task-1-Startwerte im Dashboard sind als Startzustand zu kennzeichnen. Sie duerfen Rot oder Gelb anzeigen, weil offene Kategorie, fehlende Verifikation oder Platzhalter-Annahmen sichtbar sind. Die spaetere Berechnung der Kennzahlen erfolgt in Task 3.
...
Task 1 baut `sheetOrder`, `tableBuildOrder`, alle Muss-Tabellen strukturell und die Seeds, die fuer den ersten roten oder gelben Dashboard-Nutzwertnachweis noetig sind. Task 1 darf Start-Dashboardfelder als Seed-Zustand definieren; Formeltiefe, Parserdetails und `.xlsx`-Export folgen danach.

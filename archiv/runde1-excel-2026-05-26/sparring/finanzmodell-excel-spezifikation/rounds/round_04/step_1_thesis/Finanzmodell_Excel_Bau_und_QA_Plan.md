...
- TDD beginnt mit einem vertikalen Nutzwert-Slice und erweitert danach die Breite des V1-Vertrags.
- Der Thin-Slice-Test ist konkret: Er benennt Mindest-Seeds, Tabellenkette, Startstatus und Kontrollstatus.
...
## Task 1: `workbookSpec` mit erstem Nutzwert-Gate
...
1. **Thin-Slice-Gate:** Ein minimaler Durchstich zeigt nach konkret benannten Startdaten einen roten oder gelben Dashboardstatus. Dafuer reichen Personen, Kategorie `KAT013`, ein Girokonto, ein aktives Standardszenario, Annahmen, Quelle, Importlauf, wenige Roh- und Modellumsaetze, ein Startwert fuer Liquiditaet oder Cashflow, ein Check und ein Kontrollstatus.
...
Damit ist Task 1 weder nur eine breite Tabellenabschrift noch ein zu schmaler Prototyp. Der Slice beweist den Nutzen; der Strukturvertrag verhindert, dass spaetere Tasks auf wackeligen Tabellen nachziehen.

### Grenze von Task 1

Task 1 darf den roten oder gelben Status als Start-Dashboardzustand aus `seedData` sichtbar machen. Er darf pruefen, dass die dafuer benoetigten Tabellen und Seed-Zeilen existieren und zusammen referenzierbar sind.
...
Die erste echte Berechnung gehoert in Task 3. Task 1 bereitet dafuer Spalten, Rollen, Validierungen, Kommentare und Seeds vor.
...
test("thin slice seeds form a concrete red or yellow start chain", () => {
  assert.ok(seedData.personen.some((row) => row.Person_ID === "P01"));
  assert.ok(seedData.personen.some((row) => row.Person_ID === "HH"));
  assert.ok(seedData.kategorien.some((row) => row.Kategorie_ID === "KAT013"));
  assert.ok(seedData.konten.some((row) =>
  ...
  assert.ok(seedData.quellen.length >= 1);
  assert.ok(seedData.importlaeufe.length >= 1);
  assert.ok(seedData.umsaetzeRoh.length >= 2);
  assert.ok(seedData.umsaetzeModell.some((row) =>
    row.Kategorie_ID === "KAT013"
  ));
  assert.ok(seedData.checks.some((row) => row.Check_ID === "CHK-BLD-01"));
...
test("thin slice stays structural and does not import formula logic", () => {
  assert.equal(workbookSpec.task1Scope, "structure_and_seed_contract");
  assert.equal(workbookSpec.formulaImplementationTask, 3);
});
...
- `workbookSpec.mjs` definiert `sheetOrder`, `tableBuildOrder`, `tables`, `statusSets`, `task1Scope` und `formulaImplementationTask`.
- `seedData.mjs` enthaelt Personen, Kategorien, Konten, Szenarien, Annahmen, Quellen, Importlauf, Rohumsaetze, Modellumsaetze, Kontrollstatus-Werte, Vorschlagsstatus-Werte, Agentenrollen, Check-Definitionen und `startDashboard`.
- Jede Muss-Tabelle ist strukturell beschrieben, auch wenn noch nicht jede Formel implementiert ist.
- Das Thin-Slice-Gate kann einen roten oder gelben Dashboardstatus aus Startdaten und Kontrollstatus begruenden.
...
- ein Seed-Katalog ohne referenzierbare Kette von Konto, Import, Rohumsatz, Modellumsatz, Check und Dashboard.

...
Damit ist Task 1 weder nur eine breite Tabellenabschrift noch ein zu schmaler Prototyp. Der Slice beweist Struktur, Referenzierbarkeit und Unsichtbarkeitsvermeidung; der Strukturvertrag verhindert, dass spaetere Tasks auf wackeligen Tabellen nachziehen. Der eigentliche Nutzwertnachweis fuer berechnete Liquiditaet, Cashflow und Reichweite beginnt erst mit Task 3.

### Grenze von Task 1

Task 1 darf den roten oder gelben Status als Start-Dashboardzustand aus `seedData` sichtbar machen. Er darf pruefen, dass die dafuer benoetigten Tabellen und Seed-Zeilen existieren, zusammen referenzierbar sind und der sichtbare Status aus mindestens einem vorhandenen Check mit Quellen-, Annahmen-, Import- oder Kontrollspurbezug erklaerbar ist. Er darf daraus noch keinen fachlichen Nutzen fuer Liquiditaet, Cashflow oder Reichweite behaupten.
...
test("thin slice seeds form a concrete red or yellow reference chain", () => {
  const personIds = new Set(seedData.personen.map((row) => row.Person_ID));
  ...
  assert.ok(personIds.has("P01"));
  assert.ok(personIds.has("HH"));
  ...
  assert.equal(seedData.startDashboard.Startzustand_Typ, "seeded_start_state");
...
- `seedData.checks` nutzt konkrete Einzelreferenzen (`Betroffene_Quelle_ID`, `Betroffene_Annahme_ID`, `Betroffener_Import_ID`, `Betroffener_Kontrollspur_ID`) und keine aufgeblahte generische Referenzarchitektur.
- Jede Muss-Tabelle ist strukturell beschrieben, auch wenn noch nicht jede Formel implementiert ist.
- Das Thin-Slice-Gate kann einen roten oder gelben Dashboardstatus aus Startdaten, Kontrollstatus und mindestens einem rueckgebundenen Check begruenden.
...
- ein Task-1-Test, der schon fachlichen Nutzwert von Cashflow, Liquiditaet oder Reichweite behauptet.

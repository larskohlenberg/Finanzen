...
- Task 1 startet mit einem Struktur-, Referenz- und Sichtbarkeits-Gate und endet mit einem strukturellen Vertrag fuer alle Muss-Tabellen.
- Task 1 behauptet noch keinen Nutzwert fuer Cashflow, Liquiditaet oder Reichweite; er beweist nur, dass die spaeteren Felder, IDs, Checks und Statusreferenzen sichtbar verbunden werden koennen.
...
- Check-Referenzen in `seedData.checks` bleiben konkret und knapp: Quelle, Annahme, Importlauf oder Kontrollspur, keine generische Referenzarchitektur.
...
7. Im Test explizit pruefen, dass der Slice strukturell bleibt: keine `formulas.mjs`-Importe, keine Task-Metafelder im `workbookSpec`, keine Nutzwertbehauptung, Dashboardwerte als Seed-Startzustand.
8. Im Test pruefen, dass `seedData.checks` konkrete Einzelreferenzen nutzt und keine aufgeblahte generische Check-Referenzstruktur einfuehrt.
...
- Task 1 verkauft diesen Zustand nicht als Nutzwert, sondern als Struktur-, Referenz- und Sichtbarkeitsnachweis.
...
- der Thin-Slice-Test keinen fachlichen Nutzwert vor Task 3 behauptet.

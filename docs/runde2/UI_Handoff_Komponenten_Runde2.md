...
Edge Cases: leere Tabelle (kein definierter Empty-State im Audit beobachtet — sollte laut Guideline "Keine offenen Checks"-artige, ruhige Leerzustände bekommen), sehr lange Kontonamen ("Girokonto Katrin Deutsche Bank") brechen bereits jetzt zweizeilig um.
...
- **Lange Kontonamen/Bezeichnungen**: brechen zweizeilig um (z. B. "Girokonto Katrin Deutsche Bank", "Gemeinsames Depot MLP") — aktuell funktional, aber ohne einheitliche `min-height`/`line-clamp`-Regel je Tabellenzelle.

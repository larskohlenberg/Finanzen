...
// Findings aus dem DKB-Erstimport (2026-06-09):
...
//    dateiweit EINDEUTIG ist (DKB verwendet manche Referenzen mehrfach).
...
test("intra-file: in allen Quellfeldern identische Zeilen werden BEIDE importiert", () => {
  const e = entry("2026-06-03", "27.00", "Lars", "Rueckbuchung nicht erreichbar");
  const out = runImport({
    entries: [e, { ...e }],
    konten, kategorien, kategorisierungsregeln: regeln, transaktionen: [], transfers: [],
  });
  assert.equal(out.result.written.length, 2, "beide realen Buchungen geschrieben");
...
test("wiederverwendete bank_referenz fuehrt NICHT zu falschem Merge", () => {
  // zwei real verschiedene Buchungen, von der Bank mit derselben Referenz versehen
  const out = runImport({
    entries: [
...
test("eindeutige bank_referenz ueberlebt Re-Export mit umformatiertem Zweck (ADR 0007)", () => {
  const first = runImport({
    entries: [entry("2026-05-27", "-65.93", "Telekom", "Festnetz RG 7290087996", { bank_referenz: "Zahlbeleg 394101808113" })],
    konten, kategorien, kategorisierungsregeln: regeln, transaktionen: [], transfers: [],
  });
  assert.equal(first.transaktionen[0].bank_referenz, "Zahlbeleg 394101808113", "eindeutige Referenz wird gespeichert");
  const second = runImport({
    entries: [entry("2026-05-27", "-65.93", "Telekom Deutschland GmbH", "FESTNETZ Rechnung 7290087996 (re-export)", { bank_referenz: "Zahlbeleg 394101808113" })],

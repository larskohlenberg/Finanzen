...
const regeln = [
  ...
  { regel_id: "REG-002", gegenpartei_pattern: "rewe", kategorie_id: "KAT-005", status: "aktiv", erstellt_am: "2026-06-01" },
];
...
test("offen mit Treffer wird vorgeschlagen + herkunft regel", () => {
  const t = tx({ gegenpartei: "EDEKA Mitte", kategorisierung_status: "offen" });
...
test("bestaetigt+regel mit gleicher Kategorie bleibt unveraendert", () => {
  const t = tx({ gegenpartei: "EDEKA", kategorisierung_status: "bestaetigt", kategorie_id: "KAT-003", kategorie_herkunft: "regel" });
  const out = recategorize({ transaktionen: [t], regeln });
...
test("bestaetigt+regel mit anderer konkreter Kategorie wird wiedervorgelegt", () => {
  // Bestaetigt auf KAT-003, aber das Regelwerk liefert fuer REWE jetzt KAT-005.
  const t = tx({ gegenpartei: "REWE City", kategorisierung_status: "bestaetigt", kategorie_id: "KAT-003", kategorie_herkunft: "regel" });
  const out = recategorize({ transaktionen: [t], regeln });
  const r = out.transaktionen[0];
  assert.equal(r.kategorisierung_status, "vorgeschlagen");
...
test("manuell wird nie angefasst, auch wenn eine Regel widerspricht", () => {
  const t = tx({ gegenpartei: "EDEKA", kategorisierung_status: "bestaetigt", kategorie_id: "KAT-099", kategorie_herkunft: "manuell" });
  const out = recategorize({ transaktionen: [t], regeln });
  assert.deepEqual(out.transaktionen[0], t);
...
test("abgelehnt wird nie angefasst", () => {
  const t = tx({ gegenpartei: "EDEKA", kategorisierung_status: "abgelehnt" });
  const out = recategorize({ transaktionen: [t], regeln });
  assert.deepEqual(out.transaktionen[0], t);
  assert.equal(out.report.uebersprungen, 1);
});
...
test("erhaelt unbeteiligte Felder", () => {
  const t = tx({ gegenpartei: "EDEKA", kategorisierung_status: "offen", bank_referenz: "REF-1", bemerkung: "Notiz" });
  const out = recategorize({ transaktionen: [t], regeln });
...
test("ist idempotent: zweimal laufen liefert dasselbe Ergebnis", () => {
  const txs = [
    tx({ gegenpartei: "EDEKA", kategorisierung_status: "offen" }),
    tx({ gegenpartei: "REWE", kategorisierung_status: "bestaetigt", kategorie_id: "KAT-003", kategorie_herkunft: "regel" }),
    tx({ gegenpartei: "Unbekannt", kategorisierung_status: "vorgeschlagen", kategorie_id: "KAT-003", kategorie_herkunft: "regel" }),
    tx({ gegenpartei: "EDEKA", kategorisierung_status: "bestaetigt", kategorie_id: "KAT-099", kategorie_herkunft: "manuell" }),
    tx({ gegenpartei: "EDEKA", kategorisierung_status: "abgelehnt" }),
  ];
